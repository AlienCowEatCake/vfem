...
void COCG_Smooth::init(const size_t * gi_s, const size_t * gj_s, const complex<double> * di_s,
...
complex<double> COCG_Smooth::dot_prod_nocj(const complex<double> * a, const complex<double> * b) const
{
...
double COCG_Smooth::dot_prod_self(const complex<double> * a) const
{
    double d_p = 0.0;
    for(size_t i = 0; i < n; i++)
    {
...
double COCG_Smooth::dot_prod_real(const complex<double> * a, const complex<double> * b) const
{
    double d_p = 0.0;
    for(size_t i = 0; i < n; i++)
...
void COCG_Smooth::mul_matrix(const complex<double> * f, complex<double> * x) const
{
    for(size_t i = 0; i < n; i++)
    {
...
bool COCG_Smooth::is_fpu_error(double x) const
{
    double y = x - x;
    return x != x || y != y;
}

void COCG_Smooth::solve(complex<double> * solution, const complex<double> * rp_s, double eps, size_t max_iter)
{
    eps *= eps;
    ...
    for(iter = 0; iter <= max_iter && !finished; iter++)
    {
        ...
        if(iter%10 == 0)
        {
            printf("COCG_Smooth Residual:\t%5lu\t%.3e\r", (unsigned long)iter, sqrt(residual));
            fflush(stdout);
        }
