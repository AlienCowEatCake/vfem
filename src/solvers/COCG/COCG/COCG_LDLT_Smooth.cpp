...
#endif
#include "COCG_LDLT_Smooth.h"
#include <cstdio>
#include <cmath>

void COCG_LDLT_Smooth::init(const size_t * gi_s, const size_t * gj_s, const complex<double> * di_s,
...
void COCG_LDLT_Smooth::make_LDLT_decomposition()
{
...
complex<double> COCG_LDLT_Smooth::dot_prod_nocj(const complex<double> * a, const complex<double> * b) const
{
...
double COCG_LDLT_Smooth::dot_prod_self(const complex<double> * a) const
{
    double d_p = 0.0;
    for(size_t i = 0; i < n; i++)
    {
...
double COCG_LDLT_Smooth::dot_prod_real(const complex<double> * a, const complex<double> * b) const
{
    double d_p = 0.0;
    for(size_t i = 0; i < n; i++)
...
void COCG_LDLT_Smooth::mul_matrix(const complex<double> * f, complex<double> * x) const
{
    for(size_t i = 0; i < n; i++)
    {
...
void COCG_LDLT_Smooth::solve_L(const complex<double> * f, complex<double> * x) const
{
...
void COCG_LDLT_Smooth::solve_LT(complex<double> * f, complex<double> * x) const
{
...
void COCG_LDLT_Smooth::solve_LDLT(const complex<double> * f, complex<double> * x) const
{
    solve_L(f, x);
    for(size_t i = 0; i < n; i++)
        //x[i] /= L_di[i];
...
bool COCG_LDLT_Smooth::is_fpu_error(double x) const
{
    double y = x - x;
    return x != x || y != y;
}

void COCG_LDLT_Smooth::solve(complex<double> * solution, const complex<double> * rp_s,
                             double eps, size_t max_iter)
{
    ...
    for(iter = 0; iter <= max_iter && !finished; iter++)
    {
        ...
        if(iter%10 == 0)
        {
            printf("COCG_LDLT_Smooth Residual:\t%5lu\t%.3e\r", (unsigned long)iter, sqrt(residual));
            fflush(stdout);
        }
...
//    discr = dot_prod_self(r);
    printf("COCG_LDLT_Smooth Residual:\t%5lu\t%.3e\n", (unsigned long)iter - 1, sqrt(discr / rp_norm));
...
COCG_LDLT_Smooth::COCG_LDLT_Smooth()
{
