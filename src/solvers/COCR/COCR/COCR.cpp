...
void COCR::init(const size_t * gi_s, const size_t * gj_s, const complex<double> * di_s,
...
complex<double> COCR::dot_prod_nocj(const complex<double> * a, const complex<double> * b) const
{
...
double COCR::dot_prod_self(const complex<double> * a) const
{
    double d_p = 0.0;
    for(size_t i = 0; i < n; i++)
    {
...
void COCR::mul_matrix(const complex<double> * f, complex<double> * x) const
{
    for(size_t i = 0; i < n; i++)
    {
...
bool COCR::is_fpu_error(double x) const
{
    double y = x - x;
    return x != x || y != y;
}

void COCR::solve(complex<double> * solution, const complex<double> * rp_s,
                 double eps, size_t max_iter)
{
    ...
    for(iter = 0; iter <= max_iter && !finished; iter++)
    {
        ...
        if(iter%10 == 0)
        {
            printf("COCR Residual:\t%5lu\t%.3e\r", (unsigned long)iter, residual);
            fflush(stdout);
        }
