...
#endif
#include "GMRES_Complex_Di_OpenMP.h"
#include <cstdio>
#include <cmath>
...
void GMRES_Complex_Di_OpenMP::init(const size_t * gi_s, const size_t * gj_s, const complex<double> * di_s,
...
complex<double> GMRES_Complex_Di_OpenMP::dot_prod(const complex<double> * a, const complex<double> * b) const
{
    double d_p_real = 0.0;
    double d_p_imag = 0.0;
...
double GMRES_Complex_Di_OpenMP::dot_prod_self(const complex<double> * a) const
{
    double d_p = 0.0;
...
void GMRES_Complex_Di_OpenMP::mul_matrix(const complex<double> * f, complex<double> * x) const
{
...
void GMRES_Complex_Di_OpenMP::copy_vec(const complex<double> * f, complex<double> * x) const
{
...
bool GMRES_Complex_Di_OpenMP::is_fpu_error(double x) const
{
    double y = x - x;
    return x != x || y != y;
}

void GMRES_Complex_Di_OpenMP::solve_D(const complex<double> * f, complex<double> * x) const
{
#pragma omp parallel for
    for(omp_int i = 0; i < (omp_int)n; i++)
        x[i] = f[i] / di[i];
...
void GMRES_Complex_Di_OpenMP::solve(complex<double> * solution, const complex<double> * rp_s,
...
#pragma omp parallel for
    ...
    for(iter = 0; iter <= max_iter && !finished; iter++)
    {
        ...
        //if(iter%10 == 0)
        {
            printf("GMRES_Complex_Di_OpenMP(%lu) [%d] Residual:\t%5lu\t%.3e\r", (unsigned long)m, numThreads, (unsigned long)iter, residual);
            fflush(stdout);
        }
...
GMRES_Complex_Di_OpenMP::GMRES_Complex_Di_OpenMP()
{
    d = r = w = x0 = NULL;
    H = VT = NULL;
    m = m_curr = 0;
