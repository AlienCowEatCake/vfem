...
#endif
#include "GMRES_Complex_LLT_MKL.h"
#include <cstdio>
#include <cmath>
#include <algorithm>
...
void GMRES_Complex_LLT_MKL::init(const size_t * gi_s, const size_t * gj_s, const complex<double> * di_s,
...
void GMRES_Complex_LLT_MKL::make_LLT_decomposition()
{
    cblas_zcopy(ia[m_mkl] - 1, aa, 1, L_aa, 1);
...
complex<double> GMRES_Complex_LLT_MKL::dot_prod(const complex<double> * a, const complex<double> * b) const
{
...
double GMRES_Complex_LLT_MKL::dot_prod_self(const complex<double> * a) const
{
    MKL_Complex16 * a1 = reinterpret_cast<MKL_Complex16 *>(const_cast<complex<double> *>(a));
...
void GMRES_Complex_LLT_MKL::mul_matrix(const complex<double> * f, complex<double> * x) const
{
...
void GMRES_Complex_LLT_MKL::solve_L(const complex<double> * f, complex<double> * x) const
{
...
void GMRES_Complex_LLT_MKL::solve_LT(complex<double> * f, complex<double> * x) const
{
    const MKL_Complex16 * ff = reinterpret_cast<const MKL_Complex16 *>(f);
    MKL_Complex16 * xx = reinterpret_cast<MKL_Complex16 *>(x);
...
void GMRES_Complex_LLT_MKL::solve_LTAL(const complex<double> * f, complex<double> * x, complex<double> * tmp) const
{
    copy_vec(f, x);
    solve_LT(x, x);
    mul_matrix(x, tmp);
...
void GMRES_Complex_LLT_MKL::mul_LT(const complex<double> * f, complex<double> * x) const
{
    MKL_Complex16 * in_v = reinterpret_cast<MKL_Complex16 *>(const_cast<complex<double> *>(f));
    MKL_Complex16 * out_v = reinterpret_cast<MKL_Complex16 *>(const_cast<complex<double> *>(x));
...
void GMRES_Complex_LLT_MKL::copy_vec(const complex<double> * f, complex<double> * x) const
{
...
bool GMRES_Complex_LLT_MKL::is_fpu_error(double x) const
{
    double y = x - x;
    return x != x || y != y;
}

void GMRES_Complex_LLT_MKL::solve(complex<double> * solution, const complex<double> * rp_s,
...
#pragma omp parallel for
    ...
    for(iter = 0; iter <= max_iter && !finished; iter++)
    {
        ...
        //if(iter%10 == 0)
        {
            printf("GMRES_Complex_LLT_MKL(%lu) [%d] Residual:\t%5lu\t%.3e\r", (unsigned long)m, numThreads, (unsigned long)iter, residual);
            fflush(stdout);
        }
...
GMRES_Complex_LLT_MKL::GMRES_Complex_LLT_MKL()
{
    d = r = w = t = x0 = NULL;
    H = VT = NULL;
    m = m_curr = 0;
