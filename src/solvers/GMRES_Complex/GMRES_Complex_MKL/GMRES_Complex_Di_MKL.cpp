...
#endif
#include "GMRES_Complex_Di_MKL.h"
#include <cstdio>
#include <cmath>
#include <algorithm>
...
void GMRES_Complex_Di_MKL::init(const size_t * gi_s, const size_t * gj_s, const complex<double> * di_s,
...
complex<double> GMRES_Complex_Di_MKL::dot_prod(const complex<double> * a, const complex<double> * b) const
{
...
double GMRES_Complex_Di_MKL::dot_prod_self(const complex<double> * a) const
{
    MKL_Complex16 * a1 = reinterpret_cast<MKL_Complex16 *>(const_cast<complex<double> *>(a));
...
void GMRES_Complex_Di_MKL::mul_matrix(const complex<double> * f, complex<double> * x) const
{
...
void GMRES_Complex_Di_MKL::copy_vec(const complex<double> * f, complex<double> * x) const
{
...
bool GMRES_Complex_Di_MKL::is_fpu_error(double x) const
{
    double y = x - x;
    return x != x || y != y;
}

void GMRES_Complex_Di_MKL::solve_D(const complex<double> * f, complex<double> * x) const
{
    for(size_t i = 0; i < n; i++)
        x[i] = f[i] / di[i];
}

void GMRES_Complex_Di_MKL::solve(complex<double> * solution, const complex<double> * rp_s,
...
#pragma omp parallel for
    ...
    for(iter = 0; iter <= max_iter && !finished; iter++)
    {
        ...
        //if(iter%10 == 0)
        {
            printf("GMRES_Complex_Di_MKL(%lu) [%d] Residual:\t%5lu\t%.3e\r", (unsigned long)m, numThreads, (unsigned long)iter, residual);
            fflush(stdout);
        }
...
GMRES_Complex_Di_MKL::GMRES_Complex_Di_MKL()
{
    d = r = w = x0 = NULL;
    H = VT = NULL;
    m = m_curr = 0;
