...
#endif
#include "GMRES_Complex_LDLT.h"
#include <cstdio>
#include <cmath>

void GMRES_Complex_LDLT::init(const size_t * gi_s, const size_t * gj_s, const complex<double> * di_s,
...
void GMRES_Complex_LDLT::make_LDLT_decomposition()
{
...
complex<double> GMRES_Complex_LDLT::dot_prod(const complex<double> * a, const complex<double> * b) const
{
...
double GMRES_Complex_LDLT::dot_prod_self(const complex<double> * a) const
{
    double d_p = 0.0;
    for(size_t i = 0; i < n; i++)
    {
...
void GMRES_Complex_LDLT::mul_matrix(const complex<double> * f, complex<double> * x) const
{
    for(size_t i = 0; i < n; i++)
    {
...
void GMRES_Complex_LDLT::solve_LD(const complex<double> * f, complex<double> * x) const
{
...
void GMRES_Complex_LDLT::solve_LT(complex<double> * f, complex<double> * x) const
{
...
void GMRES_Complex_LDLT::solve_LTALD(const complex<double> * f, complex<double> * x, complex<double> * tmp) const
{
    copy_vec(f, x);
    solve_LT(x, x);
    mul_matrix(x, tmp);
...
void GMRES_Complex_LDLT::mul_LT(const complex<double> * f, complex<double> * x) const
{
    for(size_t i = 0; i < n; i++)
    {
        complex<double> v_el = f[i];
...
void GMRES_Complex_LDLT::copy_vec(const complex<double> * f, complex<double> * x) const
{
    for(size_t i = 0; i < n; i++)
        x[i] = f[i];
}

bool GMRES_Complex_LDLT::is_fpu_error(double x) const
{
    double y = x - x;
    return x != x || y != y;
}

void GMRES_Complex_LDLT::solve(complex<double> * solution, const complex<double> * rp_s,
                               double eps, size_t max_iter)
{
    ...
    solve_LD(rp, w); // Правая часть у нас предобусловленная
    ...
    for(iter = 0; iter <= max_iter && !finished; iter++)
    {
        ...
        //if(iter%10 == 0)
        {
            printf("GMRES_Complex_LDLT(%lu) Residual:\t%5lu\t%.3e\r", (unsigned long)m, (unsigned long)iter, residual);
            fflush(stdout);
        }
...
GMRES_Complex_LDLT::GMRES_Complex_LDLT()
{
