...
vector3_t<T>::vector3_t(const point3_t<U> & beg, const point3_t<U> & end)
    : x(static_cast<T>(end.x - beg.x))
    , y(static_cast<T>(end.y - beg.y))
    , z(static_cast<T>(end.z - beg.z))
{}
...
{
    const T x_re = v.x.real(), x_im = v.x.imag();
    const T y_re = v.y.real(), y_im = v.y.imag();
    const T z_re = v.z.real(), z_im = v.z.imag();
