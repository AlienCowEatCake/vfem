...
void quadtree<element_type>::quadtree_node::split()
{
    ...
    const double dx = (x1 - x0) / 2.0, dy = (y1 - y0) / 2.0;
...
void quadtree<element_type>::make(double x0, double x1, double y0, double y1, std::vector<element_type> & elements)
{
    ...
    const std::size_t level_barier = static_cast<std::size_t>(std::log(static_cast<double>(total_num) + 1) / std::log(4.0)) + 1;
    const std::size_t split_constant = static_cast<std::size_t>(std::pow(static_cast<double>(total_num), 1.0 / 4.0) * 100.0);
