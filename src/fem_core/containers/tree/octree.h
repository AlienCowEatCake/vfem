...
namespace fem_core { namespace containers { namespace tree {
...
void octree<element_type>::octree_node::split()
{
    ...
    const double dx = (x1 - x0) / 2.0, dy = (y1 - y0) / 2.0, dz = (z1 - z0) / 2.0;
...
element_type * octree<element_type>::octree_node::find(double x, double y, double z) const
{
    if(point_in_node(x, y, z))
    {
        if(!sub_nodes)
        {
            const std::size_t elements_num = elements.size();
...
{
    const std::size_t total_num = elements.size();
    const std::size_t level_barier = static_cast<std::size_t>(std::log(static_cast<double>(total_num) + 1) / std::log(8.0)) + 1;
    const std::size_t split_constant = static_cast<std::size_t>(std::pow(static_cast<double>(total_num), 1.0 / 8.0) * 1000.0);
