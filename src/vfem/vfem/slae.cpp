...
{
    string solver_name, precond_name;
    size_t pos = name.find('<');
    if(pos != string::npos)
    {
        solver_name = name.substr(0, pos++);
        precond_name = name.substr(pos, name.length() - pos - 1);
    }
    delete solver;
    delete precond;
    precond = create_symmetric_complex_preconditioner(precond_name, ig, jg, di, gg, n);
    solver = create_symmetric_complex_solver(solver_name, ig, jg, di, gg, n, precond);
}
