...
do
	for imag_air in `LANG=C seq 0 1 4`
	do
		echo -e "${real_air} ${imag_air}\n" > chi_air.txt
		for real_water in `LANG=C seq 0 1 3`
		do
			for imag_water in `LANG=C seq 4 1 7`
			do
